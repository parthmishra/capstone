...
pick_rate = {}
#count = data[blueteam[3]].value_counts()
#print float(count[1])

for i in range(132):
    champ1 = data[blueteam[i]].value_counts()
    champ2 = data[redteam[i]].value_counts()
    champion_counts_1 = float(champ1[1])
    champion_counts_2 = float(champ2[1])
    rate = round((champion_counts_1 + champion_counts_2)/700.0*100.0,1)
    pick_rate[blueteam[i][:-2]] = rate

pick_data = pd.DataFrame(pick_rate.items(), columns=['Champion','Pick Rate'])
pick_data.to_csv('pick_data.csv')
...
"""
                "svm": {
                        'C':[1e3, 5e3, 1e4, 5e4, 1e5, 1.0, 0.5, 0.0001],
                        'gamma': [0.0001, 0.0005, 0.001, 0.01, 0.1, 0.5, 0.7,'auto']
                       },

                "lr": {
                        'C':[1e3, 5e3, 1e4, 5e4, 1e5, 1.0, 0.1],
                        'solver': ['newton-cg','lbfgs','liblinear']
                      },
